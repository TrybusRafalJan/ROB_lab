...
class_cnt = unique(tlab);

##imsize = 28;
##fim = zeros((imsize + 2) * 10 + 2);
##
##for clid = 1:class_cnt
##  rowid = (clid - 1) * (imsize + 2) + 3;
##  clsamples = find(tlab == clid)(1:class_cnt);
##  for spid = 1:class_cnt
...
[hlnn olnn] = crann(columns(tvec), noHiddenNeurons, class_cnt);
...
for epoch=1:noEpochs
  ...
  hid_drpout = crdrpout(noHiddenNeurons, 0.00);
  perm = randperm(rows(tvec));
  tvec = tvec(perm, :);
  tlab = tlab(perm, :);
	[hlnn olnn terr] = backprop(tvec, tlab, hlnn, hid_drpout, olnn, learningRate);
